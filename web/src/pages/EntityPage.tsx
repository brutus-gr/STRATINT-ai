...
export function EntityPage() {
  ...
  // Fetch events mentioning this entity
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/events?limit=100&sort_by=timestamp&sort_order=desc&entities=${encodeURIComponent(decodedName)}`
        );
        if (!response.ok) throw new Error('Failed to fetch events');
        const data = await response.json();

        // Filter events that actually contain this entity (case-insensitive)
        const filteredEvents = (data.events || []).filter((event: Event) =>
          event.entities.some(
            (entity) => entity.name.toLowerCase() === decodedName.toLowerCase()
          )
        );
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="border-l-4 border-terminal pl-6">
            ...
            <p className="text-sm text-smoke font-mono">
              {loading ? 'LOADING EVENTS...' : `${events.length} EVENTS MENTIONING THIS ENTITY`}
            </p>
          </div>
              ...
              <div className="border-2 border-steel bg-concrete p-16 text-center space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-fog font-mono text-sm">
                    No events mentioning "{decodedName}" in the database
                  </p>
                </div>
              </div>
            )}
